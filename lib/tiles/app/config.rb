class Tiles::Application::Configuration
  ...
  def self.new_configuration opts = {}, &blk
  	@last_config = new opts
		if block_given?
			yield @last_config.access
		else
			(blk || Proc.new {}).call(@last_config.access)
		end
  	last_config
  end
  ...
  def set_configuration
  	...
  	@configure = []
		#NOTE: Not sure if the configuration should be deleted after use
...
module Tiles::Configurable
  ...
  def configuration_method *method_list
  	_opts = method_list.pop if method_list.last.is_a? Hash
  	method_list.each do |meth|
  		raise <<-EOF.delete("\t\n") unless valid_method_name?(meth) && self.respond_to?(meth.to_sym)
				Attempting to add method #{meth} to configuration list
