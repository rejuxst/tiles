...
#  - Modify default setup for acceptabble key values
#    (i.e exclude '#' from reference names to support equation)
...
#  - Seperate Database::Base and Database::Data so that there
#    can be Database::Base that are not also Datas (This semantic should be especially
#    useful for Classes as they really shouldn't "Data" in the strict sense
...
module Database
  ...
  def self.add_database_entry_class(the_class,&blk)
		raise "This is not a class. An instance of #{the_class.class}" unless the_class.is_a? Class
...
# NOTE: All database transations should return self unless destructive
  ...
  def remove_from_db input
	# Should never be called by designer (use only for moving)
  ...
  def destroy_record_of_self
  # Destroy the record of one's self in the containing db
    db_parent.remove_from_db(self) if db_parent
		return self
  end
...
############## Misc Functions ##################################
  ...
  def inspect
		<<-EOF.gsub(/[\t \n]+/, ' ')
			#<#{self.is_a?(Class) ? (self.name) : (self.class.name)}:
					0x#{object_id.to_s(16)}
				| db: size(#{db.length}) =>
					References: #{db.count { |ky,val| val.class <= Database::Reference }}
			>
		EOF
  end
end
