require 'tiles/mixins/ncurses/ncurses_ui'
  ...
  class View < ::Ncurses::View
  	...
  	def render_mainwindow game
  		game.map.tiles.each do |k, t|
  				...
  				Ncurses.addstr((t.find_if {|_t| _t.class <= Actor} || t).ASCII)
  			end
  	end
  	...
  	def render_character_ui game
  		...
  		Ncurses.setpos 1, left_col
			hp_str = 'X' * @owner.character.hp + '-' * (@owner.character.max_hp -  @owner.character.hp)
  		Ncurses.addstr("HP: #{hp_str}")
  	end
